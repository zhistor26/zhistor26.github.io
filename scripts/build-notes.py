...
'''
    output = ROOT / 'reading' / f'{slug}.html'
...
for old in (ROOT / 'reading').glob('*.html'):
    if old not in generated: old.unlink()
notes_js = '// Generated by scripts/build-notes.py from notes/*.md.\nwindow.BLOG_NOTES = ' + json.dumps(posts, ensure_ascii=False, indent=2).replace('<', '\\u003c') + ';\n'
(ROOT / 'content/notes.js').write_text(notes_js)
index = ROOT / 'index.html'
version = hashlib.sha256(notes_js.encode()).hexdigest()[:12]
index.write_text(re.sub(r'content/notes\.js\?v=[^"\s]+', f'content/notes.js?v={version}', index.read_text()))
